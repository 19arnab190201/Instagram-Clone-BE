...
exports.followUser = BigPromise(async (req, res, next) => {
  //Search and get user's id from userName field
  const user = await User.findOne({ userName: req.body.userName });
  console.log("user accepter", user._id);
  console.log("user sender", req.user._id);
  console.log("AccType", user.accountType);
  if (!user) {
  ...
  if (user.accountType === "Public") {
    ...
    await User.findByIdAndUpdate(
      req.user._id,
      { $addToSet: { userFollowing: user._id } },
      { new: true }
    );
  } else {
    await User.findByIdAndUpdate(
